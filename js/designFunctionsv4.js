...
(function(){
   // "use strict";
    ...
    // to search any dict or array e.g. Settings.dsaServers. 
   ...
   * @returns {Array} Returns the new array after remove the element
   * @example
   *
   ...
   */    
    ...
    // send function name to be called
   ...
   getElementTopMiddleOnScreen = (el) => {
    ...
    initScreenLogo = () => {
        ...
        nodes +=        'A user interface for whole slide image channel design, analysis and registration.<br>';
...
pastMetaToInfoPanel = (item) => {
      let hostAPI = getHostApi();
      let itemBasicName = getItemRootName(itemObj.name.split(".")[0]);
      currentItemInfo.singlePlexMetaInfo = getTileSourceInfo(hostAPI, item._id, "not-ome");
      parseMetadataInfo(currentItemInfo.singlePlexMetaInfo, itemBasicName, "not-ome");  
}

pastOmeMetaToInfoPanel = (item) => {
      let hostAPI = getHostApi();
      let itemBasicName = getItemRootName(itemObj.name.split(".")[0]);
      ...
      currentItemInfo.multiPlexMetaInfo = getTileSourceInfo(hostAPI, item._id);
